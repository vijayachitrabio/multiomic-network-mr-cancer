## Script 34: [REDACTED] Replication MR
## Purpose: Replicate FinnGen-MR hits using [REDACTED] pQTL summary stats (N≈34,000)
## Run incrementally as tar files are downloaded from Synapse ([REDACTED] OLINK NGS)
##
## [REDACTED] file format (regenie output):
...
##          haplotype region). Best proxy has LOG10P=5.19 in [REDACTED] — too weak.
##          APOE [REDACTED] replication blocked by GWAS coverage, not pQTL quality.
...
##  chromosome extraction; actual GRCh37 positions come from [REDACTED] ID field)
##
...
protein_manifest <- tribble(
  ...
  ## Synapse project: [REDACTED] (PPP3) → OLINK NGS → search for gene name in
...
## ── [REDACTED] extractor ────────────────────────────────────────────────────────
extract_[redacted]_cis <- function(tar_path, chrom, pos_hg38, cis_mb = 1.5) {
...
for (i in seq_len(nrow(protein_manifest))) {
  ...
  ## Extract cis-pQTLs
  pqtl <- tryCatch(
    extract_[redacted]_cis(tar_full, pm$chr, pm$pos_hg38_approx),
  ...
  message(sprintf("    [REDACTED] MR: OR=%.4f (%.4f–%.4f), p=%.3g",
...
if (length(results) > 0) {
  ...
  out_path <- file.path(out_dir, "[redacted]_replication_mr.csv")
...
## ─────────────────────────────────────────────────────────────────────────────
## NOTES ON APOE:
##   rs429358 (APOE ε4, GRCh37 chr19:45411941, LOG10P=2069 in [REDACTED]) is the
...
##   (i) MAGMA p=0.004, (ii) observational support in [REDACTED], (iii) strong
##   coloc with glycine mediation. [REDACTED] pQTL replication blocked by GWAS.
## ─────────────────────────────────────────────────────────────────────────────
